...
static std::vector<tigrinc::Cluster> Clusters;
static tigrinc::FastaRecord Af, Bf;
static postnuc_ProcessResult* deltaProcess;
...
static bool save_delta = false;

static bool DO_DELTA = true;
static bool DO_EXTEND = true;
static bool TO_SEQEND = false;
static bool DO_SHADOWS = false;
...
bool extendForward(vector<Alignment>::iterator CurrAp,
    ...
    //  Return true if the target was reached, else false
{
    ...
    if ( Di < CurrAp->delta.size( ) )
    {
        ...
        {
            fprintf(stderr,
                    "[%s: %d] ERROR: failed to merge alignments at position %ld\n"
                    "       Please file a bug report\n",
                    __FILE__, __LINE__, CurrAp->eA);
            exit (EXIT_FAILURE);
        }
    ...
    //  data is successfully output to the file.
{
    ...
    if(save_delta)
        deltaProcess->store_header(deltaProcess->res, Af->Id, Bf->Id, Af->len, Bf->len);
    else
    ...
    for ( Ap = Alignments.begin( ); Ap < Alignments.end( ); Ap ++ )
    {
        ...
        {
            if(save_delta)
                deltaProcess->new_cluster(deltaProcess->res,
                        Ap->sA, Ap->eA,
                        Ap->sB, Ap->eB,
        ...
        {
            if(save_delta)
                deltaProcess->add_delta(deltaProcess->res, *Dp);
            else
...
{
    tigrinc::setMatrixType( tigrinc::NUCLEOTIDE );
    tigrinc::setBreakLen( 200 );
    tigrinc::setBanding( 0 );

    tigrinc::DO_DELTA = true;
    tigrinc::DO_EXTEND = true;
    tigrinc::TO_SEQEND = false;
    tigrinc::DO_SHADOWS = false;
}

void postnuc_set_parameters(postnuc_Params& params)
{

    if(params.options & POSTNUC_SET_b) tigrinc::setBreakLen( params.b );
    if(params.options & POSTNUC_SET_B) tigrinc::setBanding( params.B );
    if(params.options & POSTNUC_SET_d)
    {
        tigrinc::DO_DELTA = false;
        fprintf(stderr, "WARNING: parameter -d is not implemented!!! Setting this parameter may cause severe error\n");
    }
    if(params.options & POSTNUC_SET_e) tigrinc::DO_EXTEND = false;
    if(params.options & POSTNUC_SET_s) tigrinc::DO_SHADOWS = true;
    if(params.options & POSTNUC_SET_t) tigrinc::TO_SEQEND = true;
}
...
void postnuc_init(postnuc_Params* params /*= NULL*/)
{
    ...
    if(params)
        postnuc_set_parameters(*params);
}

void postnuc_set_ref(const char* ref_id, long int len, const char* seq)
{
    Af.Id = const_cast<char *>(ref_id);
    Af.len = len;
    Af.seq = (char *)tigrinc::Safe_malloc(sizeof(char) * (len + 2));
    strcpy(Af.seq + 1, seq);
    Af.seq[0] = 0;
    Af.seq[len + 1] = 0;
}
...
{
    Bf.Id = const_cast<char *>(qry_id);
    Bf.len = len;
    Bf.seq = (char *)tigrinc::Safe_malloc(sizeof(char) * (len + 2));
    strcpy(Bf.seq + 1, seq);
    Bf.seq[0] = 0;
    Bf.seq[len + 1] = 0;
}
...
void postnuc_add_Match(long int sA, long int sB, long int len, bool new_cluster /* = false */, bool is_forward /* = true*/)
{
    if(new_cluster)
    {
        if(Clusters.empty() || !Clusters.back().matches.empty())
            Clusters.push_back( tigrinc::Cluster() );
        Clusters.back().wasFused = false;
        Clusters.back().dirB = is_forward ? tigrinc::FORWARD_CHAR : tigrinc::REVERSE_CHAR;
    }

    tigrinc::Cluster& clu = Clusters.back();
    tigrinc::Match tmp_match;

    if(len <= 1)    return;
    tmp_match.sA = sA;
    tmp_match.sB = sB;
    tmp_match.len = len;
    clu.matches.push_back(tmp_match);
}

void postnuc_computeDelta(postnuc_ProcessResult& dp, bool auto_cleanup /* = false*/)
{
    if(!Clusters.empty() && Clusters.back().matches.empty())
        Clusters.pop_back();
    deltaProcess = &dp;
    tigrinc::save_delta = true;
    extendClusters(Clusters, &Af, &Bf, NULL);
