...
void MSA_Consensus::do_consensus(bool include_ref/* = false */)
{
    ...
    msa_lib::MSA msa;
    msa.set_NUC_matrix(1, 4, 2);
    const std::vector<std::string>& msa_aln = msa.get_alignment();
    ...
    while( ! (cur_aln >= n_alns && MI_ids.empty() && D_ids.empty() ))
    {
        ...
        if(!D_ids.empty())
        {
            ...
            if(D_size >= 2 && D_size > (total >> 1))
            {
                if(D_size == 2)
                {
                    msa.nw_alignment( alns[ D_ids[0] ].get_deletion_str(), alns[ D_ids[1] ].get_deletion_str(),
                            3, -2, -4, -4);
                }
                else
                {
                    ...
                    for(std::vector<size_t>::iterator d_it = D_ids.begin(); d_it != D_ids.end(); ++d_it)
                        msa.add_seq( alns[ *d_it ].get_deletion_str() );
                    msa.run_msa( false ); // run heuristic MSA
                }
                consensus_deletion(msa_aln, total);
            }
